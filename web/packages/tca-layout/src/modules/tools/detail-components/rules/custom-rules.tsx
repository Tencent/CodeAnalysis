...
import { getCustomRules, getCustomRulesFilters, getLanguages, deleteCustomRule, updateCustomRule, createCustomRule } from '@src/services/tools';
...
const CustomRules = ({ orgSid, toolId, toolDetail, editable, tab }: RulesProps) => {
  ...
  const getFilters = () => {
    getCustomRulesFilters(orgSid, toolId).then(res => setFilters(res));
  };
  ...
  const getListData = (offset = pageStart, limit = pageSize, otherParams = searchParams, callback?: any) => {
    ...
    setLoading(true);
    getCustomRules(orgSid, toolId, params)
      .then((response: any) => {
        // 删除最后一页的最后一条数据
  ...
  const delRule = (data: any) => {
    Modal.confirm({
      ...
      okText: t('确定删除'),
      onOk: () => {
        deleteCustomRule(orgSid, toolId, data.id).then(() => {
      ...
      />
      {
        editable && (
          <RuleModal
            ...
            onUpdate={updateCustomRule}
            onAdd={createCustomRule}
            callback={() => {
              getListData();
              getFilters();
            }}
