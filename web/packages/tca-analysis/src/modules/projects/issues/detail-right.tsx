...
const DetailRight = (props: DetailRightProps) => {
  ...
  const { orgSid, teamName, data, repoId, projectId, schemeId, issueId, curLine, callback, scrollToLine } = props;
  ...
  const getRecords = async () => {
    const issueRecords = await getIssueDetailComments(orgSid, teamName, repoId, projectId, issueId);
  ...
  return (
        <div className={style.right}>
            <div className={cn(style.topMsg, style.common)}>
                <div className={style.item}>
                    ...
                    <div className={style.msg}>
                        ...
                        <SelectBorderless
                            ...
                            onChange={(value: any) => {
                              updateIssueSeverity(orgSid, teamName, repoId, projectId, issueId, toNumber(value))
                                .then((res: any) => {
                    ...
                    <div className={style.msg}>
                        ...
                        <SelectBorderless
                            ...
                            onChange={(value: any) => {
                              updateIssueAuthor(orgSid, teamName, repoId, projectId, issueId, value)
                                .then((res: any) => {
                ...
                <div className={style.item}>
                    <h3>规则信息</h3>
                    <a onClick={() => openDetail(data.checkrule_gid)}>
                ...
                <Button disabled={!data.id} style={{ marginTop: 16 }} onClick={() => operation()}>
                    {isHandled ? '重新打开' : '忽略问题'}
                </Button>
            </div>
            <div className={cn(style.record, style.common)}>
                ...
                {issueRecords.map((item: any, index: number) => (
                    <div key={item.id} className={cn(style.item, { [style.first]: index === 0 })}>
            ...
            <RuleDetail data={ruleDetail} visible={visible} onClose={() => setVisible(false)} />
            <HandleModal visible={modalVsb} onClose={() => setModalVsb(false)} onOk={handleIssue} />
        </div>
  );
};
