...
import { Steps, Tooltip } from 'coding-oa-uikit';
...
const ScanDetail = () => {
  ...
  const { org_sid: orgSid, team_name: teamName, repoId, projectId, jobId } = useParams() as any;
                ...
                {!isEmpty(curTask.taskprocessrelation_set) && (
                  ...
                  >
                    {curTask.taskprocessrelation_set.map((item: any) => (
                      <Step
                        key={item.id}
                        title={`进程 ${item.priority} : ${PROCESS[
                          item.process
                        ] || item.process}`}
                        description={
