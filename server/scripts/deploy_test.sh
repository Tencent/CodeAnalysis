...
function diskspace_detect() {
    ...
    if ! [[ $disk_space =~ .*G$ ]]; then
        echo -e "\033[33m! your disk space $disk_space is less than 10G; please make sure you have enough space to supprt TCA"
    fi
...
function memory_detect() {
    ...
    if ! [[ $memory_avaliable_gb =~ ^([2-9]|([0-9]{2,})).* ]]; then
        echo -e "\033[33m! your avaliable memory $memory_avaliable_gb G is less than 2G; please make sure you have enough memory to supprt TCA"
    fi
}
