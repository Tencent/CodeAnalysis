...
function python_command_detect() {
    echo "make sure command python exist"
...
function python_version_detect() {
    echo "make sure python has right version"
...
function dependcy_detect() {
    echo "make sure every package is installed in right version"
    while read line
    do
        # 过滤掉不含==的行
        if ! [[ $line =~ "==" ]]; then
...
function celery_command_detect() {
    echo "make sure command celery exist"
...
function gunicorn_command_detect() {
    echo "make sure command gunicorn exist"
    if ! command -v celery &> /dev/null
    then
...
function config_detect() {
    echo "make sure enter neccessary configuration"
    while read line
    do
        # 仅监测必填配置项
...
function mysql_login_detect() {
    echo "make sure mysql configuration is correct"
    if [ $MYSQL_PORT == "" ]; then
        MYSQL_PORT="3306"
    fi
...
function redis_login_detect() {
    echo "make sure redis configuration is correct"
